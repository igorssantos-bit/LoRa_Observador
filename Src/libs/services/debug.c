...
void fnConvert_8bitHex_To_Hex_Ascii_String ( volatile uint8_t u8_value, uint8_t * pu8_dst ) {

	pu8_dst[0] = fnConvert_4bitHex_To_Ascii( u8_value/16 );
	u8_value = u8_value % 16;
	pu8_dst[1] = fnConvert_4bitHex_To_Ascii( u8_value );
	pu8_dst[2] = 0;
...
void fnConvert_16bitHex_To_Hex_Ascii_String ( volatile uint16_t u16_value, uint8_t * pu8_dst ) {

	pu8_dst[0] = fnConvert_4bitHex_To_Ascii( u16_value/4096 );
	u16_value  %= 4096;
	pu8_dst[1] = fnConvert_4bitHex_To_Ascii( u16_value/256 );
	u16_value  %= 256;
	pu8_dst[2] = fnConvert_4bitHex_To_Ascii( u16_value/16 );
	u16_value  %= 16;
	pu8_dst[3] = fnConvert_4bitHex_To_Ascii( u16_value );
	pu8_dst[4] = 0;
...
void fnDEBUG_8bit_Hex ( const char * p_char_before, const uint8_t u8_data, const char * p_char_after ) {
	...
	fnConvert_8bitHex_To_Hex_Ascii_String ( u8_data, (uint8_t*)c_buffer );
	HAL_UART_Transmit( &huart1, c_buffer, 2, MAX_DELAY );
...
void fnDEBUG_16bit_Int_Value ( const char * p_char_before, int16_t i16_data, const char * p_char_after ) {
	/*
#if ( PRODUCTION_RELEASE == true )
   return;
#endif
...
void fnDEBUG_16bit_Hex ( const char * p_char_before, const uint16_t u16_data, const char * p_char_after ) {
	char c_buffer[5];

	if ( p_char_before ) {
		HAL_UART_Transmit( &huart1, p_char_before, strlen(p_char_before), MAX_DELAY );
	}
	fnConvert_16bitHex_To_Hex_Ascii_String ( u16_data, (uint8_t*)c_buffer);
	HAL_UART_Transmit( &huart1, c_buffer, 4, MAX_DELAY );
