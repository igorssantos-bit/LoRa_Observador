...
void PrintHexBuffer( uint8_t *buffer, uint8_t size )
{
    ...
    for( uint8_t i = 0; i < size; i++ )
    {
        ...
        printf( "%02X ", buffer[i] );
    	//sprintf((char*)txBuffer, "%02X ", buffer[i] );
    	//HAL_UART_Transmit( &huart1, txBuffer, strlen( (char const*)txBuffer ), 1000 );
...
void PrintHexBuffer2( uint8_t *buffer, uint8_t size )
{
    //uint8_t newline = 0;
	uint8_t txBuffer[TX_BUF_DIM2];
...
void DisplayTxUpdate( LmHandlerTxParams_t *params )
{
    ...
    if( params->AppData.BufferSize != 0 )
    {
        ...
        //PrintHexBuffer2( params->AppData.Buffer, params->AppData.BufferSize );
